changeMavenInPath()
{
	if [[ -n $M2_HOME ]]
	then
		removeFromPath $M2_HOME/bin
	fi
	export M2_HOME=$opt/tools/apache-maven-$1
	export PATH=$PATH:$M2_HOME/bin
}

alias maven="command mvn"

color_maven()
{
  maven $* | sed -e 's/Tests run: \([^,]*\), Failures: \([^,]*\), Errors: \([^,]*\), Skipped: \([^,]*\)/[1;32mTests run: \1[0m, Failures: [1;31m\2[0m, Errors: [1;33m\3[0m, Skipped: [1;34m\4[0m/g' \
    -e 's/\(\[WARN\].*\)/[1;33m\1[0m/g' \
    -e 's/\(WARN.*\)/[0;33m\1[0m/g' \
    -e 's/\(\[INFO\].*\)/[1;32m\1[0m/g' \
    -e 's/\(\[ERROR\].*\)/[1;31m\1[0m/g' \
    -e 's/\(BUILD FAILURE.*\)/[1;31m\1[0m/g' \
    -e 's/\(FAILURE!.*\)/[1;31m\1[0m/g' \
    -e 's/\(BUILD SUCCESS.*\)/[1;37m\1[0m/g' \
    -e 's/\(SUCCESS.*\)/[1;37m\1[0m/g' 
}

alias mvn="color_maven"

m3()
{
	changeMavenInPath 3.1.0
	mvn $*
}

m3st()
{
	m3 -DskipTests $*
}

function setupGrails
{
	if [[ -n $GRAILS_HOME ]]
	then
		removeFromPath $GRAILS_HOME/bin
	fi

	if [ ! -d $opt/sdk/grails-$1 ] 
	then
		source ~/scripts/install-grails.sh
		installGrails $1
	fi

	export GRAILS_HOME=$opt/sdk/grails-$1
	export PATH=$PATH:$GRAILS_HOME/bin
	echo "Using grails ${1}"
}

function grails203
{
	setupGrails 2.0.3
}
